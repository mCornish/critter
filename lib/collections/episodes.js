...
Episodes.allow({
    update: function(userId, episode) {
        return ownsDocument(userId, episode) || isAdmin(userId);
    },
    remove: function(userId, episode) {
        return ownsDocument(userId, episode) || isAdmin(userId);
    }
});
...
Episodes.deny({
    update: function(userId, episode, fieldNames) {
        ...
        return (_.without(fieldNames, 'number', 'name', 'cast', 'description', 'airDate', 'videoId').length > 0);
    }
});
...
Meteor.methods({
    episodeInsert: function(episodeAttributes) {
        ...
        check(episodeAttributes, {
            ...
            cast: [String],
            description: String,
            airDate: String, // Maybe make this a date later
            videoId: String
        });
